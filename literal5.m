...
N=16; %longitud del codigo gold
In=8; %indice del codigo gold
H = comm.GoldSequence('SamplesPerFrame',N,'Index',In); %objeto que permite la creaciond el codigo gold
secGold=H()'; %secuencia del codigo gold obtenida
...
for i=1:N1 %lazo for anidado para realizar la expansion
    for j=1:N
        Senal_extendida(1,k)=xor(datos(1,i),secGold(1,j)); %uso de la funcion xor para el calculo del resultado ente la secuencia gold y los datos
...
Gold_extendida = rectpulse(secGold(1,:),10); %extension de la secuencia Gold
subplot(4,2,3); 
stairs(Gold_extendida,'y'); %grafico de la secuencia Gold
axis([0 length(Gold_extendida) -0.5 1.5])
title(' Secuencia Gold '); %titulo
...
for j=1:N %lazo for que permite realizar la multiplicacion de los datos recibidos con la secuencia Gold
    SenalRecibida(1,j) = xor(sedemod(1,k),secGold(1,j)); %producto xor entre la senal demodulada y la secuencia Gold
...
Gold_extendida = rectpulse(secGold(1,:),10); %extension de la secuencia Gold extendida
subplot(4,2,6); 
