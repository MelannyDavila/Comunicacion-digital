...
modulacion='QPSK'; %modulacion: BPSK, QPSK, 8PSK, 16QAM, 32QAM, 64QAM

fft_long=512; %tamano de ifft/fft

PC=64; %tamano del prefijo ciclico
...
metodos_mod={'BPSK','QPSK','8PSK','16PSK','4QAM','16QAM','32QAM','64QAM'};
...
if orden_mod==2 || orden_mod==3 %si se utiliza 2 o 3 bits
    mod_ind=2^(orden_mod-1);
    n=0:pi/mod_ind:2*pi-pi/mod_ind;
    in_phase=cos(n+pi/4); %portadora en fase
    quadrature=sin(n+pi/4); %portadora en cuadratura
    symbol_book=(in_phase+quadrature*1i)';
end
...
if orden_mod==4
    ...
    n=0:pi/mod_ind:2*pi-pi/mod_ind;
    in_phase=cos(n+pi/8);
    quadrature=sin(n+pi/8);
    symbol_book=(in_phase+quadrature*1i)';
end

%Modulaciones 16QAM y 64QAM
if orden_mod==6 %si se utilizan 4 o 6 bits
...
%Modulacion 32QAM
if orden_mod==5 %si utilizan 5 bits
    mod_ind=6;
    in_phase=repmat(linspace(-1,1,mod_ind),mod_ind,1); %portadora en fase
    quadrature=repmat(linspace(-1,1,mod_ind)',1,mod_ind); %portadora en cuadratura
    symbol_book=in_phase(:)+quadrature(:)*1i;
